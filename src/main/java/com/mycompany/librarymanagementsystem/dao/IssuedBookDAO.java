...
public boolean issueBook(int bookId, int memberId, Timestamp dueDate) {
    ...
    try (Connection conn = DBConnection.getConnection()) {
        ...
        try {
            ...
            try (PreparedStatement statusStmt = conn.prepareStatement(
                    "UPDATE books SET status = CASE WHEN available_copies <= 0 THEN 'BORROWED' ELSE 'AVAILABLE' END " +
                    "WHERE id = ?")) {
...
public List<IssuedBook> getNotReturnedBooksByBookId(int bookId) {
    List<IssuedBook> notReturnedBooks = new ArrayList<>();
    String sql = "SELECT i.*, b.title as book_title, m.name as member_name " +
                 "FROM issued_books i " +
                 "JOIN books b ON i.book_id = b.id " +
                 "JOIN members m ON i.member_id = m.id " +
                 "WHERE i.return_status = 'NOT_RETURNED' AND i.book_id = ? " +
                 "ORDER BY i.date_issued DESC";
